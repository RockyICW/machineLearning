...
BOOL Cdemo_MFCDlg::OnInitDialog()
{
	...
	m_strIPAddress = L"192.168.1.110";//Default IP Address
...
void Cdemo_MFCDlg::showBvhBoneInfo(SOCKET_REF sender, BvhDataHeader* header, float* data)
{
	...
	for (int curSel = 38; curSel < BVHBoneCount; curSel++)
	{
	...
	strOut.append(csvDoc);
	strOut.append("\r\n");



	//  Save the CSV to a file:
	success = csv.SaveFile("data.csv");
	if (success != true) {
		strOut.append(csv.lastErrorText());
		strOut.append("\r\n");
	}
...
void Cdemo_MFCDlg::saveCSV(int curSel,float angX, float angY, float angZ)
{
	CkString strOut;
	CkCsv csv;
	...
	*/
	...
	/*
	string x = "angX_" + std::to_string(curSel);
	string y = "angY_" + std::to_string(curSel);
	string z = "angZ_" + std::to_string(curSel);

	success = csv.SetColumnName(0, "Finger");
	success = csv.SetColumnName(1 + (curSel - 38)*3 , x.c_str());
	success = csv.SetColumnName(2 + (curSel - 38)*3, y.c_str());
	success = csv.SetColumnName(3 + (curSel - 38)*3, z.c_str());
	*/
	/**/

	int row = csv.get_NumRows();
	if (curSel != 38) row -= 1;
	bool success = csv.SetCell(row, 0, "0");
	success = csv.SetCell(row , csv.GetNumCols(row), std::to_string(angX).c_str());
	success = csv.SetCell(row , csv.GetNumCols(row), std::to_string(angY).c_str());
	success = csv.SetCell(row , csv.GetNumCols(row), std::to_string(angZ).c_str());
